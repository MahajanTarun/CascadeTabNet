...
def border(table,image):
    ...
    # points=str(table[0])+","+str(table[1])+" "+str(table[2])+","+str(table[3])+" "+str(table[2])+","+str(table[3])+" "+str(table[2])+","+str(table[1])
    points = [str(table[0]), str(table[1]), str(table[2]), str(table[3]), str(table[2]), str(table[3]), str(table[2]), str(table[1])]
    Coords["points"] = points

    # *Tcoords = etree.Element("Coords",)
    tableXML["Coords"] = Coords
    ...
    for box in final:
        if box[0]>table[0]-5 and box[1]>table[1]-5 and box[2]<table[2]+5 and box[3]<table[3]+5:
            ...
            cell["end-col"] = str(end_col)
            cell["end-row"] = str(end_row)
            cell["start-col"] = str(start_col)
            cell["start-row"] = str(start_row)
            ...
            cell_points = []
            cell_points.append(one)
            cell_points.append(two)
            cell_points.append(three)
            cell_points.append(four)

            coords = {}
            # coords = etree.Element("Coords", points=one+" "+two+" "+three+" "+four)
            coords["points"] = cell_points
